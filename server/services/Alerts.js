...
const loadPagerDutyAlerts = () => PagerDutyApi.incidents({"statuses": ["acknowledged", "triggered"],limit: 200})
  .then(incidents => {
    ...
    return Promise.all(incidents.map(i => 
      PagerDutyApi.incidentNotes(i.id).then(notes => { i.notes = notes; return i })
    ))
  }).then(incidents => 
    // load alerts for each incident and extend each alert with the correspondent incident.
    Promise.all(incidents.map(i => 
      PagerDutyApi.incidentAlerts(i.id).then(alerts => alerts.map(a => {a.incident = i; return a }) )
    ))     
  ...
  .catch(error => console.error(`[${moment().format('DD.MM.YYYY HH:mm')}] PAGERDUTY LOADER: error: `, error.message)) 
;
...
const buildAcknowledgements = (incident) => {
  ...
  if(incident.acknowledgements) {
    incident.acknowledgements.forEach(ack => 
      result.push({at: new Date(ack.at), user: {name: ack.acknowledger.summary} })
    )
  }
  return result
}
...
utils.doPeriodical({intervalInSeconds: 300, immediate: true}, loadPagerDutyAlerts)
