...
const testNotes = [
  {
    ...
    content:
      "Incident was acknowledged on behalf of sven.blaschke@sap.com. time: 2019-10-22 08:31:08.929396719 +0000 UTC",
...
describe("get", () => {
  ...
  describe("alerts", () => {
    ...
    it("returns test alerts", () => {
      alerts.forEach((alert) => {
        const testAlert = testAlerts.find(
          (a) => a.annotations.summary === alert.annotations.summary
        )
  ...
  describe("labelValues", () => {
    ...
    it("contains white listed labels", () => {
      expect(Object.keys(labelValues).sort()).toMatchObject([
        "cluster",
        "region",
        "service",
        "severity",
...
describe("pagerDutyAlertKey", () => {
  it("builds pagerduty alert key", () => {
    ...
    expect(key).toEqual(
      `${alert.severity || ""}-${details.Service || ""}-${details.Tier || ""}-${
        details.Region || ""
      }-${details.Context || ""}`
    )
  })
...
describe("alertKey", () => {
  it("builds alert key", () => {
    ...
    expect(key).toEqual(
      `${details.severity || ""}-${details.service || ""}-${
        details.tier || ""
