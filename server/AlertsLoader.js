...
const updateCachedAckedAlerts = (alerts) => {
...
const updateCachedAlerts = (alerts) => {
  const {items,counts,labelValues}  = extendAlerts(sortAlerts(alerts))
...
const loadAckedAlerts = () => {
  PagerDutyApi.resolvedAlerts()
    .then(alerts => {
      console.log(`PAGER DUTY->ALERTS LOADER [${new Date()}]: receive new alerts from API`)

      const newCacheString = JSON.stringify(alerts)
  
      if(!_cachedAckedAlerts.cacheString || _cachedAckedAlerts.cacheString !== newCacheString) {
        _cachedAckedAlerts.cacheString = newCacheString
        updateCachedAckedAlerts(alerts)
      }
    }) 
    .catch(error => {
...
const start = (onUpdate) => {
  ...
  utils.doPeriodical({intervalInSeconds: 300, immediate: true}, loadAckedAlerts)
}
