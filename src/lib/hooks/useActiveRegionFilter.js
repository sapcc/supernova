...
const useActiveRegionFilter = () => {
  const state = useGlobalState()
  const { alerts, categories } = state

  return useMemo(() => {
    let items =
      alerts.counts && alerts.counts.region
        ? Object.keys(alerts.counts.region).slice()
        : []
    items = items.filter((item, index) => items.indexOf(item) === index)

    // if a landscape category is active then filter regions
    const landscapeCategories = categories.items.filter(
      (c) => c.active && c.area === "landscape" && c.match_re.region
    )
...
export default useActiveRegionFilter
