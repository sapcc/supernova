...
const useActiveCategoryCounts = ({ counts, categories }) =>
  useMemo(() => {
    const activeLandscapeCategory = categories.find(
      (c) => c.area === "landscape" && c.active
    )
    ...
    if (
      ...
      category &&
      category[activeLandscapeCategory.name]
    ) {
      for (let subCategoryName in category[activeLandscapeCategory.name]
        .category) {
        category[subCategoryName] = {
          ...category[activeLandscapeCategory.name].category[subCategoryName],
        }
      }
    }

    const activeCategories = categories.filter(
      (c) => c.active && c.area !== "landscape"
    )
    ...
    activeCategories.forEach((c) => {
      ...
      for (let regionName in region) {
        ...
        ;["critical", "warning", "info"].forEach((severity) => {
          region[regionName][severity] = Math.min(
            region[regionName][severity] || 0,
            categoryRegion[severity] || 0
          )
          ;[
            `${severity}Handled`,
            `${severity}Silenced`,
            `${severity}Acked`,
          ].forEach((handledSeverity) => {
            region[regionName][handledSeverity] = Math.min(
              region[regionName][handledSeverity] || 0,
              categoryRegion[handledSeverity] || 0,
              region[regionName][severity]
            )
          })
        })
      }
...
export default useActiveCategoryCounts
