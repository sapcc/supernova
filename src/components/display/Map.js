...
const AnnotationContent = ({ region, dx, dy, counts }) => {
  ...
  return (
    <React.Fragment>
      <svg className="map-region" x={xy.x} y={xy.y} height={totalRegionHeight}>
        <rect
        ...
        {["critical", "warning", "info"].map((severity, index) => (
          <svg
            x={index * severityWidth}
            y={regionInfoHeight}
            height={severityInfoHeight}
            className={counts[severity] > 0 ? "" : "allclear"}
            key={severity}
          >
            <rect
            ...
            >
              <tspan>
                {openAlertCount(counts[severity], counts[`${severity}Handled`])}
              </tspan>
            </text>
              ...
              (region, index) =>
                LOCATIONS[region] && (
                  <Marker
                    ...
                    marker={{
                      coordinates: [
                        LOCATIONS[region].lon,
                        LOCATIONS[region].lat,
                      ],
                    }}
                  >
              ...
              (region, index) =>
                LOCATIONS[region] && (
                  <Annotation
                    key={index}
                    stroke="#000"
                    dx={LOCATIONS[region].dx}
                    dy={LOCATIONS[region].dy}
